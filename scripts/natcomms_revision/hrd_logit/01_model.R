...
# Use only subjects present in the cleaned HRD dataset (data/cleaned/HRD.csv)
# to ensure consistency with the metacognition pipeline
CLEANED_HRD <- file.path(BASE_DIR, "data", "cleaned", "HRD.csv")
if (!file.exists(CLEANED_HRD)) stop("Cleaned HRD not found: ", CLEANED_HRD)
cleaned_subjects <- unique(read.csv(CLEANED_HRD)$subject)
n_before <- length(unique(df_intero$subject))
df_intero <- df_intero %>%
  filter(subject %in% cleaned_subjects)
n_after <- length(unique(df_intero$subject))
message(sprintf("Subject alignment: %d -> %d (dropped %d not in cleaned HRD)",
