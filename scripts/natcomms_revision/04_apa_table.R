...
doc <- read_docx() %>%
  ...
  body_add_par("") %>%
  body_add_par("Note. B = unstandardized coefficient (log-odds scale); SE = standard error; CI_95 = 95% confidence interval. Continuous predictors are mean-centered within subject. Reference level for Drug is Placebo. Model: Response ~ drugs * listenBPM + responseBPM + nTrials + (RE).",
               style = "Normal")
