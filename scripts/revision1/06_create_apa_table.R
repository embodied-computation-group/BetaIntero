#!/usr/bin/env Rscript
# Create APA-Style Results Table for BetaIntero Revision 1
# Extracts fixed effects from the latest model and saves as a Word document and PDF.
...
results_formatted <- tidy_res %>%
  mutate(
    ...
    Predictor = case_when(
      ...
      term == "listenBPM_scaled" ~ "Listen BPM (Scaled)",
      term == "responseBPM_scaled" ~ "Response BPM (Scaled)",
      term == "nTrials_scaled" ~ "Trial Number (Scaled)",
      term == "visitvisit_0002" ~ "Visit 2 (vs Visit 1)",
      term == "visitvisit_0003" ~ "Visit 3 (vs Visit 1)",
      term == "drugsBISO:listenBPM_scaled" ~ "Bisoprolol × Listen BPM",
      term == "drugsPROP:listenBPM_scaled" ~ "Propranolol × Listen BPM",
      TRUE ~ term
    ),
    # Format numbers
...
ft <- flextable(results_formatted) %>%
  ...
  set_caption(
    caption = as_paragraph(
      ...
      as_chunk("\n"),
      as_chunk("Fixed Effects for Interoception Choice Model (Drug Interaction + Trial Control)")
    )
  )

# Generate Timestamp for filename
TIMESTAMP <- format(Sys.time(), "%Y%m%d_%H%M%S")
doc_filename <- sprintf("apa_table_results_%s.docx", TIMESTAMP)
...
doc <- read_docx() %>%
  ...
  body_add_par("") %>%
  body_add_par("Note. B = unstandardized coefficient (log-odds scale); SE = standard error; CI_95 = 95% confidence interval. Predictors are standardized. Reference level for Drug is Placebo. Model: Response ~ drugs * listenBPM + responseBPM + nTrials + (RE).", 
               style = "Normal")
...
# Also save as CSV
csv_filename <- sprintf("apa_table_results_%s.csv", TIMESTAMP)
...
# Save as PDF
# Note: Direct save_as_image to PDF can be flaky depending on package versions.
# We will save as HTML first (using htmltools to avoid Pandoc dependency), then use webshot2 to convert to PDF.

if (!require("htmltools")) install.packages("htmltools")

html_filename <- sprintf("apa_table_results_%s.html", TIMESTAMP)
...
pdf_filename <- sprintf("apa_table_results_%s.pdf", TIMESTAMP)
