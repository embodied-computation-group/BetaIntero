...
transformed parameters{
  ...
  for(n in 1:T){
    ...
    alpha[n] = dot_product(X_alpha[n,], alpha_p[S_id[n],]) + prediction[n] * gm[N+1];
    
    beta[n] = exp(dot_product(X_beta[n,], beta_p[S_id[n],]) + prediction[n] * gm[N+2]);
    
    lapse[n] = inv_logit(dot_product(X_lapse[n,], lapse_p[S_id[n],]) + prediction[n] * gm[N+3]) / 2;
...
model{
  ...
  target += normal_lpdf(gm[4] | 0,5); //global mean of difference between alpha
  
  target += normal_lpdf(gm[5] | 0,5); //global mean average HRV on alpha
  
  target += normal_lpdf(gm[6] | 2.25,0.5); //global mean of beta
  
  target += normal_lpdf(gm[7] | 0, 3); //global mean of difference between beta
  
  target += normal_lpdf(gm[8] | 0, 3); //global mean of difference between beta
  
  target += normal_lpdf(gm[9] | 0, 3); //global mean of difference between beta
  
  target += normal_lpdf(gm[10] | 0, 3); //global mean of average HRV on beta
  
  target += normal_lpdf(gm[11] | -5.5,1.5); //global mean of lapse

  target += normal_lpdf(gm[12] | 0,2); //global mean of difference between lapse

  target += normal_lpdf(gm[13] | 0,2); //global mean of difference between lapse

  target += normal_lpdf(gm[14] | 0,5); //global mean of prediction effect on alpha
  target += normal_lpdf(gm[15] | 0,3); //global mean of prediction effect on beta
  target += normal_lpdf(gm[16] | 0,3); //global mean of prediction effect on lapse
  ...
  target += normal_lpdf(tau_u[6] | 0.25,0.5);
  target += normal_lpdf(tau_u[7] | 0, 2);
  target += normal_lpdf(tau_u[8] | 0, 2);
  target += normal_lpdf(tau_u[9] | 0, 2);
  target += normal_lpdf(tau_u[10] | 0, 2);

  target += normal_lpdf(tau_u[11] | 2.5,1);
  target += normal_lpdf(tau_u[12] | 0,3);
  target += normal_lpdf(tau_u[13] | 0,3);
